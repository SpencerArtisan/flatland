...
class SceneSpec extends AnyFlatSpec with should.Matchers {
  ...
  "A scene" should "render if it has no size" in {
  ...
  it should "not render a dot outside the world" in {
    Scene.from(World().add(Dot(100), Coord(-1, -1)), 2, 2).render() should be("..\n..")
  }

  it should "render a single dot in the top left" in {
    Scene.from(World().add(Dot(100), Coord(0, 0)), 2, 2).render() should be("*.\n..")
  }

  it should "render a single dot in the bottom right" in {
    Scene.from(World().add(new Dot(100), Coord(1, 1)), 2, 2).render() should be("..\n.*")
  }
  ...
  it should "render a circle" in {
    Scene.from(World().add(Circle(100, 1), Coord(1, 1)), 3, 3)
      .render() should be(".*.\n***\n.*.")
  }

  it should "render a rectangle" in {
    Scene.from(World().add(Rectangle(100, 2, 1), Coord(1, 1)), 3, 3)
      .render() should be("...\n.**\n...")
  }

  it should "render a rectangle rotated through 90 degrees" in {
    Scene.from(World().add(Rectangle(100, 3, 1), Coord(1, 1), Math.PI / 2), 3, 3)
      .render() should be(".*.\n.*.\n.*.")
  }
  ...
  it should "rotate a rectangle after placement" in {
    val s = for {
      ...
      scene = Scene.from(w, 3, 3).render()
    } yield scene
    s should be(Right(".*.\n.*.\n.*."))
  }

  it should "render a rectangle which fills the entire world" in {
    Scene.from(World().add(Rectangle(100, 5, 5), Coord(1, 1)), 3, 3)
      .render() should be("***\n***\n***")
  }

  it should "not render a rectangle which is outside the world" in {
    Scene.from(World().add(Rectangle(100, 5, 5), Coord(100, 100)), 3, 3)
      .render() should be("...\n...\n...")
  }
}